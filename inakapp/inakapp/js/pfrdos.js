$(document).delegate('#pfrdos', 'pagecreate', function() {
	var dom = {
		"facturasrecibidas": function(arrayfacturas){
			...
			for (i in arrayfacturas){
				...
				var celdaiva= "<td class=\"textoizda\"><a id=\"fiva" + arrayfacturas[i].id + "\" href=\"#popupiva\" data-rel=\"popup\" data-position-to=\"window\" class=\"\" data-transition=\"pop\">" + arrayfacturas[i].iva + "</a></td>";
				//var celdaiva = "<td class=\"textoizda\">" + arrayfacturas[i].iva+ "</td>";
				...
				$("#fiva" + arrayfacturas[i].id).on("tap", function(event) {
					modid = this.getAttribute("id");
					modid = modid.replace("__", "_").replace("fiva", "");
					tabla = "tb_facturasrecibidas";
				});
		...
		"guardariva" : function() {
			$("#popupivaboton").on("tap", function() {
				console.log("hola");
				var campo = "IVA";
				var modivaporcval = $("#modiva").val();
				cambio = "iva";
				//websql.enviarmod(campo, modivaporcval);
				websql.facturasrecibidas();
	...
	var websql = {
		...
		"enviarmod" : function(campo, modval) {
			...
			$.ajax({
				...
				success : function() {
					window.location.href = "pfrdos.php";
				}
			});
		},
