...
$resultadotrabajosdia = mysqli_query($con, "SELECT ID_HORAS, FECHA, PROYECTO, FASE, DETALLE, HORAS, USUARIO, BOLSA FROM tb_horas WHERE FECHA ='" .$trabajofecha."' ORDER BY PROYECTO ");
while ($row = mysqli_fetch_array($resultadotrabajosdia)) {
$arraytrabajosusuario=array("id"=>utf8_encode($row['ID_HORAS']),"fecha"=>utf8_encode($row['FECHA']),"usuario"=>utf8_encode($row['USUARIO']), "proyecto"=>utf8_encode($row['PROYECTO']),"fase"=>utf8_encode($row['FASE']), "detalle"=>utf8_encode($row['DETALLE']),"horas"=>$row['HORAS'],"bolsa"=>$row['BOLSA']);
