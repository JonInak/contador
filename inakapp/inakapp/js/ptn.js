$(document).delegate('#ptn', 'pagecreate', function() {
	var dom = {
			...
			$.ajax({
				...
				success : function(respuestatrabajos) {
					for (i in respuestatrabajos){
						if (respuestatrabajos[i].codigo[0]!= "A" && respuestatrabajos[i].codigo[0]!= "E"){
							var codigofloat = parseFloat(respuestatrabajos[i].codigo.substring(0,4));
							console.log(codigofloat);
							if(ultimocodigo<codigofloat){
								ultimocodigo = parseFloat(respuestatrabajos[i].codigo.substring(0,4));
							}
						}	
					}
